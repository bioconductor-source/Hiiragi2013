...
\renewcommand{\prefix}{\jobname}
\newcommand{\myincfig}[4]{%
  \begin{figure}[tbp]
  \begin{center}
  \includegraphics[width=#2]{\prefix-#1}
...
\title{{\Large Executable Analysis Document Supporting:}\\
  \textbf{Cell-to-cell gene expression variability followed by reinforcement 
  progressively segregates lineages in the early mouse embryo}\\
...
data preprocessing (``normalisation'') by the RMA
...
@
%
\texttt{x} is an \emph{\Sexpr{class(x)}} object that contains the
normalized data for the \Sexpr{ncol(x)} arrays.
<<showx>>=
...
%-------------------------------------------------- 
We define a grouping of the samples and an associated colour map, 
% as from Yusuke's email of 12.4.2013, 10:36h)
which will be used in the plots in this report.
%
<<sampleColours1>>=
...
signal across samples is a more specific indicator of a transcripts presence, than the
...
@ 
%
\myincfig{figHistSds}{0.5\textwidth}{Histogram of
standard deviation of each probe set's signal}{across the \Sexpr{ncol(xe)} samples.}
...
\myincfig{figmas5calls}{\textwidth}{Present/absent calls.}{The barplot shows, for each of the
...
the square root of the minimal sum of the squared differences of $u$
and all column permutations of $v$, where $u$ and $v$ are the cluster membership matrices.
...
boxplot(lapply(ce, `[[`, "agreements"), ylab="agreements", col=colours)
...
@
%
\myincfig{figClue1}{\textwidth}{Cluster stability analysis.}{Left:
...
\begin{enumerate}
  ...
  \item the WT EPI 4.5 and WT PE 4.5 cells.
\end{enumerate}
...
@ 
%
\myincfig{figClue2}{\textwidth}{Cluster stability analysis with E3.5/E4.5 WT and E4.5 FGF4-KO samples.}{Top 
  row: Results of the cluster stability analysis with the E3.5 (EPI), E3.5 (PE), E4.5
  (FGF4-KO) samples. Shown on the $y$-axis is the membership probability 
...
<<saveRandomSeed, cache=TRUE, echo=FALSE, results=hide>>=
oldseed <- .Random.seed
@
%
Although in the 2-dimensional PCA projection in Figure~\ref{figmds-2} dots representing FGF4-KO E3.5 cells appear to overlap with ones representing WT E3.25 samples, a cluster stability analysis analogous to that from the previous section indicates that they form a distinct population (Figure~\ref{figClue3}).
%
<<ce3,cache=TRUE>>=
...
@
%
<<figClue3,fig=TRUE,width=6,height=3>>=
par(mfrow=c(1,2))
  for(j in seq_len(2))
    plot(cl_membership(resampledSampleSet$consensus)[,j], 
     ylab=paste0("P(cluster ", j, ")"), xlab="Samples", col=x$sampleColour[sampleSets], pch=16, cex=1.5)

@ 
%
\myincfig{figClue3}{.66\textwidth}{Cluster stability analysis with E3.25 WT and E3.5 FGF4-KO samples.}{Results of the cluster stability analysis with the E3.25 WT and E3.5 FGF4-KO samples. The $y$-axis shows the membership probability in the two clusters. The actual group membership of the samples (known but not used by the clustering algorithm) is indicated by the colours.}
...
<<figHeatmapClue,fig=TRUE,width=8,height=12>>=
ngenes = 100
...
<<restoreRandomSeed, cache=TRUE, echo=FALSE, results=hide>>=
.Random.seed <- oldseed
@
\myincfig{figHeatmapClue}{\textwidth}{Heatmap of the E3.25 WT and E3.5 FGF4-KO samples.}{Data of the 100 genes with the highest variance; the population of FGF4-KO samples clusters together.}

Even though a few individual cells from E3.25 WT do indeed seem to have similar expression profiles as ones from E3.5 FGF4-KO, the populations are not the same.
...
To control the influence of the parameter \Robject{ngenes}, we call the
...
image(x=seq_len(nrow(clusters)),
      y=seq_len(ncol(clusters)),
      z=clusters,
      col=c("#f0f0f0", "#000000"),
      ylab="ngenes", xlab="samples", yaxt="n")
text(x=0, y=seq_len(ncol(clusters)), paste(ngenes), xpd=NA, adj=c(1, 0.5))
@
%
\myincfig{figClusters}{0.5\textwidth}{The influence of the
  parameter \Robject{ngenes} on the clustering result.}{Each of the 22
...
@
We can check how the microarray-data driven clustering compares with
...
@
%
We can see that the clustering perfectly agrees with Yusuke's labeling. \emph{Note}: the
...
@
\myincfig{figIndepFiltSetup}{0.5\textwidth}{Determination of the cutoff for independent
  filtering.}{Each dot corresponds to one of the \Sexpr{nrow(xForClustering)} features on the array, the $x$-axis
...
ord = order(adjp)
# FDRcutoff = 0.1
# differentially = ord[ adjp[ord]<=FDRcutoff & !is.na(adjp[ord]) ]
# length(differentially)
...
%
<<figHeatmapAll, fig=TRUE, width=13, height=14>>=
myHeatmap(x[differentially, x$genotype=="WT"])
<<figHeatmapAllColl, fig=TRUE, width=13, height=12>>=
myHeatmap(x[differentially, x$genotype=="WT"], collapseDuplicateFeatures=TRUE)
<<figHeatmap35, fig=TRUE, width=7, height=14>>=
myHeatmap(xForClustering[differentially,])
<<figHeatmap35Coll, fig=TRUE, width=7, height=12>>=
...
\myincheatmap{figHeatmap35}{0.7}{only the WT arrays from E3.5}{\Sexpr{length(differentially)}}{For some genes, multiple features are shown}
\myincheatmap{figHeatmap35Coll}{0.7}{only the WT arrays from E3.5}{\Sexpr{length(differentially)}}{Duplicate features per gene were averaged}
...
\myincfig{figIF45}{0.5\textwidth}{Determination of the cutoff for independent
  filtering.}{Analogous to Figure~\ref{figIndepFiltSetup}, but for E4.5.}
<<BH45>>=
passfilter = which(varianceRank<=nFilt)
adjp = rep(NA_real_, nrow(x))
...
heatmap Figure~\ref{figdifferentiallyE325toE35}.
...
<<figdifferentiallyE325toE35, width=8, height=25, fig=TRUE>>=
myHeatmap(x[deE325toE35, samples], collapseDuplicateFeatures=TRUE)
@
%
\myincfig{figdifferentiallyE325toE35}{7.5cm}{Heatmap of
...
@
%
\myincfig{figProjection}{8cm}{Projection of sample expression profiles on
...
<<doPCA2,cache=TRUE>>=
thepca = prcomp(t(exprs(xwte)), center=TRUE, scale=TRUE)
pcatrsf = function(x) (scale(t(exprs(x)), center=TRUE, scale=TRUE)  %*% thepca$rotation)
...
%
\myincfig{figmds-1}{.6\textwidth}{PCA plot, using WT samples.}{The colour code is as in
  Figure~\ref{figProjection}.}
\myincfig{figpcaloadings}{\textwidth}{Sorted loadings (coefficients) of the first two
...
\myincfig{figmds-2}{.6\textwidth}{PCA
...
\myincfig{figmds-3}{\textwidth}{Same as Figure~\ref{figmds-2}, with labels
...
\myincfig{figmds-4}{\textwidth}{Same as Figure~\ref{figmds-2}, with labels
...
\myincfig{fighmko}{\textwidth}{Heatmap.}{The
...
@
%
\myincfig{figFGF4expression}{\textwidth}{FGF4 expression.}{The
...
@
\myincfig{figFGF4jitter}{0.6\textwidth}{FGF4 expression (microarray signal) in WT and KO samples.}{}
...
@
%
\myincfig{figCorFgf4Dist}{0.6\textwidth}{Relationship between
...
@
%
\myincfig{figFGF4variab}{0.6\textwidth}{Variability of different groups of
...
@
%
\myincfig{figNumberOfCells-100}{\textwidth}{Distance of the E3.25 WT samples to the mean profile of FGF4-KO.}
  {The 100 features with highest overall variance were used.}
\myincfig{figNumberOfCells-1000}{\textwidth}{Distance of the E3.25 WT samples to the mean profile of FGF4-KO.}
...
@
%
\myincfig{figHeatmapKO}{\textwidth}{Heatmap of all arrays from groups
...
@
%
\myincfig{figIndepFiltDeWtKo}{\textwidth}{Differentially expressed genes between
...
\myincfig{fig2DWTKO}{0.5\textwidth}{Differentially expressed genes between
...
\myincfig{figFGF4ProbesInKO}{\textwidth}{The probes for FGF4.}{As expected, the signal
...
@ 
%
\myincfig{figCtrls}{\textwidth}{Behaviour of some control probe sets.}{}
...
pws = c("04010", "04070", "04150", "04310", "04330", "04340", "04350", "04370", "04630") 
  # "04390", "04064", "04066", "04151", "04075", "04668"
@ 
%
...
@ 
\myincfig{figShowPws}{\textwidth}{Differentially expressed genes between
...
@
%
\myincfig{fig-JSD}{\textwidth}{Jensen-Shannon divergences}{JSD is
...
\myincfig{fig-ExemplaryGenes}{\textwidth}{Boxplots of the
...
@ 
%
\myincfig{figHeatmapthclasses}{9.5cm}{Heatmap of all classes}{of
...
\myincfig{fighm1}{\textwidth}{Heatmap of the qPCR data set.}{
...
\myincfig{fighm2}{\textwidth}{Heatmap of the qPCR data for the 7 selected genes.}{}
...
\myincfig{fighm3}{\textwidth}{Heatmap for the 7 selected genes and the
...
\myincfig{figthr}{\textwidth}{Visualisation of the qPCR data for the seven
...
par(mfrow=c(nrow(sxq), 1), mai=c(0.1, 0.7, 0.01, 0.01))
...
@ 
%
\myincfig{fighmd}{\textwidth}{Heatmap for the discretized values.}{Rows and
...
\myincfig{figboot}{.6\textwidth}{Distribution of bootstrap-resampled optimal
...
The MDS plot in Fig.~\ref{mdsplot} was produced using the following code. 
...
@
\myincfig{mdsplot}{.6\textwidth}{MDS plot}


\section{Correlation between Fgf ligands and Fgf receptors}

Figure~\ref{figure5b} was produced using the following code. 
<<figure5b,width=10,height=2.5,fig=TRUE>>=
...
@ 
%
\myincfig{figure5b}{\textwidth}{Figure 5b}
