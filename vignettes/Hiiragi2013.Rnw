...
BiocStyle::latex(short.fignames=TRUE)
@ 
...
@ 
%
\incfig{figHistSds}{0.5\textwidth}{Histogram of
standard deviation of each probe set's signal}{across the \Sexpr{ncol(xe)} samples.}
...
par(mfrow=c(1,2))
  for(j in seq_len(2))
    plot(cl_membership(resampledSampleSet$consensus)[,j], 
     ylab=paste0("P(cluster ", j, ")"), xlab="Samples", col=x$sampleColour[sampleSets], pch=16)

@ 
%
\incfig[b!]{figClue3}{.66\textwidth}{Cluster stability analysis with E3.25 WT and E3.5 FGF4-KO samples.}{Results of the cluster stability analysis with the E3.25 WT and E3.5 FGF4-KO samples. The $y$-axis shows the membership probability in the two clusters. The actual group membership of the samples (known but not used by the clustering algorithm) is indicated by the colours.}
...
@
\incfig{mdsplot}{.6\textwidth}{MDS plot}
...
@ 
%
\incfig{figure5b}{\textwidth}{Figure 5b}
